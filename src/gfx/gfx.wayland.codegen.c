...
main(int argc, char **argv)
{
  ...
  ParsedXml wayland_protocol = load_and_parse_xml(parse_arena, Str8Lit(DATA_DIR"/xml/wayland.xml"));
  ParsedXml xdg_shell_protocol = load_and_parse_xml(parse_arena, Str8Lit(DATA_DIR"/xml/xdg-shell.xml"));
  ParsedXml linux_dmabuf_v1_protocol = load_and_parse_xml(parse_arena, Str8Lit(DATA_DIR"/xml/linux-dmabuf-v1.xml"));
  ...
  String8 message_opcodes = str8_join(codegen, &message_opcode_list, 0);
  String8 request_functions = str8_join(codegen, &request_function_list, 0);
  ...
  String8 code = str8_join(codegen, &code_list, 0);

  printf("concatenated code\n");

  os_write_entire_file(code, Str8Lit(SRC_DIR"/gfx/gfx.wayland.generated.c"));
