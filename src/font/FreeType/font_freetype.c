        ...
        for EachRange(glyph_idx, U32, glyph_idx_rng)
        {
          ...
          if(ft_load_glyph_result == FT_Err_Ok)
          {
            ...
            if(ft_render_glyph_result == FT_Err_Ok)
            {
              ...
              {
                glyph->bitmap = arena_push_array(arena, U32, glyph->width*glyph->height);

                U32 *dest_row = glyph->bitmap + glyph->stride*(glyph->height-1);
                U8 *src_row = ft_bitmap.buffer;
                for(U32 row_idx = 0; row_idx < ft_bitmap.rows; ++row_idx)
                {
                  U32 *dest_pixels = dest_row;
                  U8 *src_pixels = src_row;
                  for(U32 col_idx = 0; col_idx < ft_bitmap.width; ++col_idx)
                  {
                    U8 src = *src_pixels++;
